...
import sys, tkinter.ttk, random, _thread, time, tkinter.scrolledtext, socket, logging, os, argparse, urllib3, string, json, urllib.request, configparser, subprocess, sqlite3, webbrowser, re, ssl, signal
...
parser.add_argument("-s", "--startport", help="connect port", type=int, required=False)
parser.add_argument("-m", "--multi", type=int, required=False, help="-m 5   run 5 different TOR instances", choices=range(1, 101))
parser.add_argument("-t", "--port", type=int, required=False, help="start port")
...
parser.add_argument("-r", "--control", type=int, required=False, help="control port")
parser.add_argument("-f", "--dnstor", type=int, required=False, help="dns port")
if linux:
...
class IpChanger(Tk):
    ...
    def __init__(self):
        ...
        self.controlport = args.control
        self.control = args.control
        self.appPORT = 14999
        if args.startport:
            self.appPORT = args.startport
            
        if args.port:
            self.proxy = args.port
        else:
            self.proxy = args.port
            ...
            if self.newWindow:
                ...
                try:
                    ...
                    if args.multi is not None:
                        ...
                        exec('config[\'127.0.0.1:args.port\'] = {}')
                        if eval("self.useBridges_0.get()") == 1:
                            exec('config[\'127.0.0.1:args.port\'][\'bridge\'] = \'yes\'')
                        else:
                            exec('config[\'127.0.0.1:args.port\'][\'bridge\'] = \'no\'')
                        if eval("self.checkvarUnique_0.get()") == 1:
                            exec('config[\'127.0.0.1:args.port\'][\'unique\'] = \'yes\'')
                        else:
                            exec('config[\'127.0.0.1:args.port\'][\'unique\'] = \'no\'')
                            exec("self.proxyblacklist_0 = 0")
                        lang = eval('self.tor_0.get()')
                        exec('config[\'127.0.0.1:args.port\'][\'country\'] = \'' + str(lang) + '\'')
                    with open(self.settingsIni, 'w') as configfile:
                        config.write(configfile)
    ...
    def blacklistWindow(self, proxy=None):
        if proxy is None:
            ...
            proxy = args.port + proxy
        cport = 5950 + proxy
        languagenumber = proxy - args.port
...
usage: ipchanger.exe [-a AUTO] [-d] [-m 1-100] [-p] [-c COUNTRY] [-b] [-n] [-u]
                        ...
                        try:
                            ...
                            if args.multi is None:
                                if not port == args.port:
                                    reply += bytes('u cant use port %s without ipchanger -m, only args.port\r\n' % port, 'utf-8')
                                    port = args.port

                            cislo = port - args.port
    ...
    def tor(self):
        ...
        if args.multi is not None:
            ...
            if windows:
                subprocess.Popen(r'Tor/tor.exe -f %s/torrc %s --CookieAuthentication 0 --SocksPolicy "accept *" --HashedControlPassword "%s" --ControlPort %s --SocksPort 0.0.0.0:%s --DataDirectory %s --log %s --AvoidDiskWrites 1 --SafeLogging 0 --GeoIPExcludeUnknown 1 --GeoIPFile Tor/geoip --GeoIPv6File Tor/geoip6 --DNSport %s --AutomapHostsSuffixes .onion --AutomapHostsOnResolve 1 %s' % (self.data, self.bridge, self.get_controlPasswordHash('0'), self.control, self.proxy, self.data, self.tordebuglevel, args.dnstor, options), startupinfo=info, stdout=open(file, 'a'))
            elif linux:
                os.system(r'(authbind --deep tor -f %s/torrc %s --CookieAuthentication 0 --SocksPolicy "accept *" --HashedControlPassword "%s" --ControlPort %s --SocksPort 0.0.0.0:%s --DataDirectory %s --log %s --AvoidDiskWrites 1 --SafeLogging 0 --GeoIPExcludeUnknown 1 --GeoIPFile Tor/geoip --GeoIPv6File Tor/geoip6 --DNSport %s --AutomapHostsSuffixes .onion --AutomapHostsOnResolve 1 %s | tee %s ) > /dev/null &' % (self.data, self.bridge, self.get_controlPasswordHash('0'), self.control, self.proxy, self.data, self.tordebuglevel, args.dnstor, options, file))
            self.control = self.control + 1
            self.proxy = self.proxy + 1
            self.b = self.b + 1
            self.data = "Data/tordata%s" % self.b
        ...
        while not timeout == tout and key == self.ident:
            ...
            timeout = timeout + 1
            try:
                status = self.torControl(args.control, self.get_controlPassword(0), "GETINFO status/bootstrap-phase")
                ...
                circuitstatus = self.torControl(args.control, self.get_controlPassword(0),
    ...
    def GetExternalIP(self, proxy=None, url=None):
        if url is None:
            url = self.ipresolver
        file = "Data/tordata%s/ip.txt" % (proxy - args.port)
    ...
    def newIP(self, port=None):
        ...
        if self.bezi == 1:
            ...
            if port is not None:
                instanci = 1
                proxy = int(port)
                newcontrolport = args.control - args.port + int(port)
                self.controlport = int(newcontrolport)
            identify = proxy - args.port
            for i in range(instanci):
                try:
